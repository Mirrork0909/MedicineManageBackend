            ...
            try
            {
                ...
                if (list != null)
                {
                    for (int i = 0; i < completeContractDTO.stockInDTOs.Count; i++)
                    {
                        ...
                        var isInStock = Db.Queryable<MEDICINE_STOCK>().Where((ms) => ms.MEDICINE_ID == completeContractDTO.stockInDTOs[i].medicine_id
                        && completeContractDTO.stockInDTOs[i].batch_id == ms.BATCH_ID).First();
                        if (isInStock == null)
                        {
                            return false;
                        }
                        ...
                        {
                            isInStock.AMOUNT += completeContractDTO.stockInDTOs[i].in_num;
                        ...
                        stock_in.IN_NUM = completeContractDTO.stockInDTOs[i].in_num;
                        stock_in.IN_TIME = completeContractDTO.stockInDTOs[i].in_time;
